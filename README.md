...
- **Offline cache** — Loads cached data immediately when internet is unavailable
- **Network-aware** — Coalesces overlapping refresh requests, skips live API calls offline, refreshes on reconnect
...
- **오프라인 캐시** — 인터넷 연결 실패 시 캐시 데이터를 즉시 불러와 표시
- **네트워크 감지** — 중복 새로고침 합치기, 오프라인 시 실시간 호출 건너뛰기, 복구 시 즉시 갱신
